...
async function getForecast() {
    ...
    try {
        ...
        if (dateInput) {
            ...
            //today.setHours(0, 0, 0, 0); // Important: Normalize dates for accurate comparison
            //selectedDate.setHours(0, 0, 0, 0); // Important: Normalize dates for accurate comparison

            if (selectedDate < today) { // Correct comparison: strictly less than today
                forecastData = await fetchHistoricalForecast(latitude, longitude, selectedDate, forecastDays);
            } else {
                forecastData = await fetchForecast(latitude, longitude, forecastDays); // Use forecast for current/future
            }
        } else {
            forecastData = await fetchForecast(latitude, longitude, forecastDays); // No date selected, use forecast
        }
...
function displayDailyBarChart(forecastData) {
    const barChartType = document.getElementById("bar-chart-type").value;
    const chartContainer = document.getElementById('daily-bar-chart-container');
    const existingSelect = document.getElementById("bar-chart-type"); // Get the existing select

    // Clear only the canvas, keep the select
    ...
    if (barChartType === "both" || barChartType === "solar") {
        const dailySolar = dailyLabels.map(date => dailyData[date].solar);
        datasets.push({
            label: 'Daily Solar Energy (Wh/m²)',
            data: dailySolar,
    ...
    if (barChartType === "both" || barChartType === "wind") {
        const dailyWind = dailyLabels.map(date => dailyData[date].wind);
        datasets.push({
            label: 'Daily Wind Energy (Wh/m²)',
            data: dailyWind,
    ...
    if (datasets.length === 0) {
        ...
        noDataMessage.textContent = "No energy data selected for display.";
...
function displayChart(timestamps, solarPower, windPower) {
    ...
    const chartType = document.getElementById("bar-chart-type").value;
